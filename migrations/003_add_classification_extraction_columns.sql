...
ALTER TABLE documents ADD COLUMN IF NOT EXISTS categorie VARCHAR(50);
...
ALTER TABLE documents ADD COLUMN IF NOT EXISTS metoda_clasificare VARCHAR(50) CHECK (
    metoda_clasificare IN ('filename_regex', 'text_rules', 'ai', 'text_override', 'vision')
);
ALTER TABLE documents ADD COLUMN IF NOT EXISTS review_status VARCHAR(50) NOT NULL DEFAULT 'OK' CHECK (
    review_status IN ('OK', 'REVIEW', 'NEEDS_CHECK')
);
...
ALTER TABLE extraction_results ADD COLUMN IF NOT EXISTS data_expirare VARCHAR(100);
ALTER TABLE extraction_results ADD COLUMN IF NOT EXISTS companie VARCHAR(200);
ALTER TABLE extraction_results ADD COLUMN IF NOT EXISTS producator VARCHAR(200);
ALTER TABLE extraction_results ADD COLUMN IF NOT EXISTS distribuitor VARCHAR(200);
...
COMMENT ON COLUMN documents.metoda_clasificare IS 'Classification method used: filename_regex, text_rules, ai, text_override, vision';
COMMENT ON COLUMN documents.review_status IS 'Review status: OK, REVIEW, NEEDS_CHECK';
...
COMMENT ON COLUMN extraction_results.extraction_model IS 'AI model used for extraction (e.g., google/gemini-2.0-flash-001)';
